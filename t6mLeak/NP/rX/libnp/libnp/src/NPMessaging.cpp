...
static void (__cdecl * randomStringCB)(const char*);

LIBNP_API void LIBNP_CALL NP_RegisterRandomStringCallback(void (__cdecl * callback)(const char*))
{
	randomStringCB = callback;
}

static void Messaging_HandleRandomString(INPRPCMessage* message)
{
	RPCStorageSendRandomStringMessage* msg = (RPCStorageSendRandomStringMessage*)message;

	if (randomStringCB)
	{
		std::string str = msg->GetBuffer()->randomstring();

		randomStringCB(str.c_str());
	}
}
...
void Messaging_Init()
{
	...
	RPC_RegisterDispatch(RPCStorageSendRandomStringMessage::Type, Messaging_HandleRandomString);
}