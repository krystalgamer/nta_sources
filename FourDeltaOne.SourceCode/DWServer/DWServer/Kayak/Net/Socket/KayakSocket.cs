...
namespace Kayak
{
    class DefaultKayakSocket : ISocket
    {
        ...
        public void Connect(IPEndPoint ep)
		{
            ...
            DWServer.Log.Debug("KayakSocket: connecting to " + ep);
            this.socket = new SocketWrapper(ep.Address.AddressFamily);
			
            socket.BeginConnect(ep, iasr => 
            {
            	DWServer.Log.Debug("KayakSocket: connected to " + ep);
                ...
                scheduler.Post(() =>
                {
                    ...
                    if (error != null)
                    {
                        ...
                        DWServer.Log.Debug("KayakSocket: error while connecting to " + ep);
                        RaiseError(error);
                    }
                    else
                    {
        ...
        internal void BeginRead()
        {
            ...
            while (true)
            {
                ...
                DWServer.Log.Debug("KayakSocket: reading.");
                ...
                try
                {
                    ar0 = socket.BeginReceive(inputBuffer, 0, inputBuffer.Length, ar =>
                    {
                        ...
                        scheduler.Post(() =>
                        {
                            DWServer.Log.Debug("KayakSocket: receive completed async");
                ...
                DWServer.Log.Debug("KayakSocket: receive completed sync");
        ...
        {
            DWServer.Log.Debug("KayakSocket: " + (sync ? "" : "a") + "sync read " + read);
        ...
        void HandleReadError(Exception e)
        {
            ...
            DWServer.Log.Debug("KayakSocket: read error");
            ...
            if (e is SocketException)
            {
                ...
                {
                    DWServer.Log.Debug("KayakSocket: peer reset (" + socketException.ErrorCode + ")");
                    PeerHungUp();
                    return;
                }
            }
        ...
        void PeerHungUp()
        {
            DWServer.Log.Debug("KayakSocket: peer hung up.");
        ...
        public bool Write(ArraySegment<byte> data, Action continuation)
        {
            ...
            buffer.Add(data);
            DWServer.Log.Debug("KayakSocket: added " + data.Count + " bytes to buffer, buffer size was " + bufferSize + ", buffer size is " + buffer.Size);
        ...
        void HandleSendResult(int written, bool sync)
        {
            ...
            DWServer.Log.Debug("KayakSocket: Wrote " + written + " " + (sync ? "" : "a") + "sync, buffer size is " + buffer.Size);
            ...
            if (shutdownSocket)
            {
                DWServer.Log.Debug("KayakSocket: shutting down socket after send.");
                socket.Shutdown();
            }
        ...
        public void End()
        {
            DWServer.Log.Debug("KayakSocket: end");
            ...
            if (shutdownSocket)
            {
                DWServer.Log.Debug("KayakSocket: shutting down socket on End.");
                socket.Shutdown();
            }
        ...
        {
            DWServer.Log.Debug("KayakSocket: raising OnError");
            del.OnError(this, e);
        ...
        void RaiseClosed()
        {
            DWServer.Log.Debug("KayakSocket: raising OnClose");
            del.OnClose(this);
        }
    }
}
