...
namespace Kayak
{
    class DefaultKayakServer : IServer
    {
        ...
        public IDisposable Listen(IPEndPoint ep)
        {
            ...
            DWServer.Log.Debug("KayakServer will bind to " + ep.ToString());
            ...
            DWServer.Log.Debug("KayakServer bound to " + ep.ToString());
        ...
        void Close()
        {
            ...
            DWServer.Log.Debug("Closing listening socket.");
            listener.Close();
        ...
        {
            //DWServer.Log.Debug("Connection " + socket.id + ": closed (" + connections + " active connections)");
        ...
        void AcceptNext()
        {
            try
            {
                DWServer.Log.Debug("KayakServer: accepting connection");
                listener.BeginAccept(iasr =>
                {
                    DWServer.Log.Debug("KayakServer: accepted connection callback");
                    Socket socket = null;
                    ...
                    scheduler.Post(() =>
                    {
                        DWServer.Log.Debug("KayakServer: accepted connection");
                        if (error != null)
        ...
        void HandleAcceptError(Exception e)
        {
            ...
            DWServer.Log.Debug("Error attempting to accept connection.");
