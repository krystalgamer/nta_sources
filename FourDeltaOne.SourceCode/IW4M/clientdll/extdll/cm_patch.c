...
static int CM_GridPlane(int gridPlanes[MAX_GRID_SIZE][MAX_GRID_SIZE][2], int i, int j, int tri)
{
	...
	// should never happen
	Com_Printf(0, "WARNING: CM_GridPlane unresolvable\n");
	return -1;
}
							   ...
							   int i, int j, int which)
{
	...
	for(k = 0; k < facet->numBorders; k++)
	{
		...
		else
		{
			...
			Com_Printf(0, "WARNING: CM_SetBorderInward: mixed plane sides\n");
			facet->borderInward[k] = qfalse;
			if(!debugBlock)
			{
				debugBlock = qtrue;
	...
	for(axis = 0; axis < 3; axis++)
	{
		for(dir = -1; dir <= 1; dir += 2, order++)
		{
			...
			if(i == facet->numBorders)
			{
				...
				{
					Com_Printf(0, "ERROR: too many bevels\n");
				}
			...
			for(dir = -1; dir <= 1; dir += 2)
			{
				...
				if(i == facet->numBorders)
				{
					...
					for(k = 0; k < facet->numBorders; k++)
					{
						...
						{
							Com_Printf(0, "WARNING: bevel plane already used\n");
						}
					}
