...
bool Auth_DisplayLoginDialog(const wchar_t** usernameP, const wchar_t** passwordP)
{
	...
	info.pszCaptionText = L"Authentication for T5M";
	info.pszMessageText = L"Please log in to use T5M. Use your fourDeltaOne.net forum credentials.";
...
bool Auth_PerformSessionLogin(const char* username, const char* password)
{
	...
	if (curl)
	{
		...
		curl_easy_setopt(curl, CURLOPT_USERAGENT, "T5M");
...
#if PRE_RELEASE_DEMO
skipLogin:
	bool result = Auth_PerformSessionLogin("xnp", "xnpxnpxnpxnp");
