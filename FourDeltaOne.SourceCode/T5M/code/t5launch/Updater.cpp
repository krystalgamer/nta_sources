...
bool Updater_RunUpdate(int numCaches, ...)
{
	...
	int result = DL_RequestURL("http://content.operation2.net/t5/content/caches.xml", cachesFile, sizeof(cachesFile));

	if (result != 0)
	{
		MessageBox(NULL, va(L"An error (%i) occurred while checking the game version. Check if http://content.operation2.net/t5/ is available in your web browser.", result), L"O\x448\x438\x431\x43A\x430", MB_OK | MB_ICONSTOP);
		return false;
	}
	...
	{
		result = DL_RequestURL(va("http://content.operation2.net/t5/content/%s/info.xml", cache.name.c_str()), cachesFile, sizeof(cachesFile));
	...
	for (auto& filePair : queuedFiles)
	{
		...
		if (fileOutdated)
		{
			const char* url = va("http://content.operation2.net/t5/content/%s/%s%s", cache.name.c_str(), file.name.c_str(), (file.compressed) ? ".xz" : "");
