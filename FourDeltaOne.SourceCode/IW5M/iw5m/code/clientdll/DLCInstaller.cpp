...
#define CHECK_STATUS(x) \
	...
	CHECK_STATUS(RegCreateKey(HKEY_CURRENT_USER, "SOFTWARE\\Classes\\.iw5dlc", &iw5dlc));
	CHECK_STATUS(RegSetValueEx(iw5dlc, NULL, 0, REG_SZ, (BYTE*)"IW5M.DLCPack", 13));
	CHECK_STATUS(RegCloseKey(iw5dlc));

	CHECK_STATUS(RegCreateKey(HKEY_CURRENT_USER, "SOFTWARE\\Classes\\IW5M.DLCPack", &iw5dlc));
	CHECK_STATUS(RegSetValueEx(iw5dlc, NULL, 0, REG_SZ, (BYTE*)"IW5M DLC Package", 17));
	CHECK_STATUS(RegCloseKey(iw5dlc));

	CHECK_STATUS(RegCreateKey(HKEY_CURRENT_USER, "SOFTWARE\\Classes\\IW5M.DLCPack\\shell\\open\\command", &iw5dlc));
...
void DLCInstaller_Install(const wchar_t* path)
{
	...
	int mbResult = MessageBoxA(NULL, va("Do you want to install the '%s' content pack for IW5M to the MW3 game directory '%s'?", dlcName, dpath), "IW5M DLC", MB_YESNO | MB_ICONQUESTION);
	...
	{
		MessageBoxA(NULL, va("The '%s' IW5M content pack has been successfully installed.", dlcName), "IW5M DLC", MB_OK | MB_ICONINFORMATION);
	}
	else
	{
		MessageBoxA(NULL, va("The '%s' IW5M content pack did not install successfully. Error code 0x%x was returned.", dlcName, result), "IW5M DLC", MB_OK | MB_ICONERROR);
	}
}
