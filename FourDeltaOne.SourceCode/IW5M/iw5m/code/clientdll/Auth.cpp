...
bool Auth_DisplayLoginDialog(const wchar_t** usernameP, const wchar_t** passwordP)
{
	...
	info.pszMessageText = L"Please log in to use IW5M. Use your fourDeltaOne.net forum credentials.";
...
size_t UDataReceived(void *ptr, size_t size, size_t nmemb, void *data)
{
	...
	{
		MessageBoxA(NULL, va("This demo version (%d) has expired. Please visit http://fourdeltaone.net/ (or run iw4_updater.exe) to obtain a new version (%d).", BUILDNUMBER, version), "Oshibka", MB_OK);
		ExitProcess(0x1);
	}
...
void DisableOldVersions()
{
	...
	{
		char url[255];
		_snprintf(url, sizeof(url), "http://iw5.prod.fourdeltaone.net/content/version.txt");
...
void Auth_OpenDLCStore()
{
	...
	{
		char url[255];
		_snprintf(url, sizeof(url), "http://iw5.prod.fourdeltaone.net/content/dlc.txt");
