...
private:
	...
	void TryLoadLibraries()
	{
		...
		snprintf(szLibraryPath, sizeof(szLibraryPath) - 1, "%s/%s", m_szSteamPath, "vstdlib_s.dll");
		m_pVStdLib = new DynamicLibrary(szLibraryPath);

		snprintf(szLibraryPath, sizeof(szLibraryPath) - 1, "%s/%s", m_szSteamPath, "tier0_s.dll");
		m_pTier0 = new DynamicLibrary(szLibraryPath);
	...
	DynamicLibrary* m_pTier0;
	DynamicLibrary* m_pVStdLib;
};
