...
namespace IWI8SkyTool
{
    ...
    {
        internal static byte[] DecodeARGB4(byte[] data, int width, int height)
        {
            ...
            {
                pixData[j] = (byte)(data[i] >> 4);
                pixData[j + 1] = (byte)(data[i] & 0x7F);

                pixData[j + 2] = (byte)(data[i + 1] >> 4);
                pixData[j + 3] = (byte)(data[i + 1] & 0x7F);
                j += 4;
            }
