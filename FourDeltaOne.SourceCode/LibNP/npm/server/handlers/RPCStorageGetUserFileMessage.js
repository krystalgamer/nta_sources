...
module.exports = function(data, id)
{
    ...
    if (connection.npID != state.npID && state.fileName.indexOf('statsCom') == -1 && state.fileName.indexOf('profile') == -1)
    {
        replyWithError(2, state);
        return;
    }
    ...
    server.instance.database.xnp_files().get(fileID, {}, function(err, body)
    {
        if (err)
        {
            ...
            {
				if (state.fileName == "globalstaaaaaaaaaatsCompressed")
				{
					logger.warn('restricted T5 for ' + connection.npID);
					return;
				}
